...
After a server graph has been opened successfully at least once, notnote keeps a local replica of its notes and settings in IndexedDB. Later PWA launches restore that replica immediately. When a connection is available, notnote keeps the startup overlay in place until it has compared the cached revisions with the server and downloaded only changed notes, so stale cached text is not exposed for editing. Without a connection, the installed PWA can open the replica, edit existing notes, and create new pages or journals. Changes are applied immediately to the local index and placed in a persistent synchronization queue.
...
| Delete block and children | `⌘/Ctrl + Alt + Backspace` |
...
Templates support `{{date}}` (`yyyy-MM-dd`), `{{time}}` (`HH:mm`), `{{today}}` (a reference to today's journal), `{{page}}` (the current page title), and `{{cursor}}` (the initial caret position). Unknown placeholders remain unchanged. Template definitions are ordinary Markdown and follow the graph across local, remote, and offline use. A block and its descendants can also be saved from the bullet's **Make a template** context-menu action.
...
Use `/upload` inside a graph block to select any file. notnote stores it in the graph root’s `assets/` directory, preserves the original name when available, and inserts a Markdown link; images, audio, and video use image Markdown automatically. Move the pointer over an embedded image and select the expand icon in its lower-right corner to open the full-screen viewer; selecting the image itself keeps the normal Markdown editing behavior. Close the viewer with the × button, the backdrop, or `Escape`. Audio and video references written with `![](...)` are shown as native players. Trusted iframe embeds from YouTube, Vimeo, Spotify, and SoundCloud are also rendered. If a filename already exists, notnote appends `-1`, `-2`, and so on.
