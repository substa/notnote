...
A server-backed browser stores a replica and pending operations in IndexedDB. On later launches the replica is restored immediately; while the startup overlay still protects the editor, an online client reconciles it with a lightweight path/revision manifest and downloads only changed notes. Existing-note edits and new pages can be queued offline. Pending operations use a separate IndexedDB store, Web Locks serialize same-origin synchronization, and BroadcastChannel complements server events between browser windows. The server replica is refreshed before current-version queued writes are replayed. Conflicting and legacy queued writes are never forced automatically: their local text becomes a recovery draft while the authoritative server version remains visible. The server graph remains authoritative after synchronization.
